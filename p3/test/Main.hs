...
transferMoney_1 = do
  ...
  atomically $ registerUser "Salavat" bank
  SalavatAccountId <- atomically $ createAccount "Salavat" USD bank
  atomically $ putMoney "Salavat" SalavatAccountId RUB 10000 bank
  ...
  -- act
  isOK <- atomically $ transferMoney "Salavat" SalavatAccountId "Danil" danilAccountId 10 bank

  -- assert
  (users, bankAccount) <- atomically $ readTVar bank

  let Just SalavatAccountsVar = Map.lookup "Salavat" users
  SalavatAccounts <- atomically $ readTVar SalavatAccountsVar
  let Just SalavatAccount = IntMap.lookup SalavatAccountId SalavatAccounts
...
transferMoney_2 = do
  ...
  -- act
  isOK <- atomically $ transferMoney "Salavat" SalavatAccountId "Danil" danilAccountId 1000 bank
